// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from ros2_learning_msg:srv/SetCommand.idl
// generated code does not contain a copyright notice
#include "ros2_learning_msg/srv/detail/set_command__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

// Include directives for member types
// Member `robot_name`
#include "rosidl_runtime_c/string_functions.h"
// Member `parameters`
#include "rosidl_runtime_c/primitives_sequence_functions.h"

bool
ros2_learning_msg__srv__SetCommand_Request__init(ros2_learning_msg__srv__SetCommand_Request * msg)
{
  if (!msg) {
    return false;
  }
  // robot_name
  if (!rosidl_runtime_c__String__init(&msg->robot_name)) {
    ros2_learning_msg__srv__SetCommand_Request__fini(msg);
    return false;
  }
  // command_type
  // parameters
  if (!rosidl_runtime_c__float__Sequence__init(&msg->parameters, 0)) {
    ros2_learning_msg__srv__SetCommand_Request__fini(msg);
    return false;
  }
  return true;
}

void
ros2_learning_msg__srv__SetCommand_Request__fini(ros2_learning_msg__srv__SetCommand_Request * msg)
{
  if (!msg) {
    return;
  }
  // robot_name
  rosidl_runtime_c__String__fini(&msg->robot_name);
  // command_type
  // parameters
  rosidl_runtime_c__float__Sequence__fini(&msg->parameters);
}

bool
ros2_learning_msg__srv__SetCommand_Request__are_equal(const ros2_learning_msg__srv__SetCommand_Request * lhs, const ros2_learning_msg__srv__SetCommand_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // robot_name
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->robot_name), &(rhs->robot_name)))
  {
    return false;
  }
  // command_type
  if (lhs->command_type != rhs->command_type) {
    return false;
  }
  // parameters
  if (!rosidl_runtime_c__float__Sequence__are_equal(
      &(lhs->parameters), &(rhs->parameters)))
  {
    return false;
  }
  return true;
}

bool
ros2_learning_msg__srv__SetCommand_Request__copy(
  const ros2_learning_msg__srv__SetCommand_Request * input,
  ros2_learning_msg__srv__SetCommand_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // robot_name
  if (!rosidl_runtime_c__String__copy(
      &(input->robot_name), &(output->robot_name)))
  {
    return false;
  }
  // command_type
  output->command_type = input->command_type;
  // parameters
  if (!rosidl_runtime_c__float__Sequence__copy(
      &(input->parameters), &(output->parameters)))
  {
    return false;
  }
  return true;
}

ros2_learning_msg__srv__SetCommand_Request *
ros2_learning_msg__srv__SetCommand_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ros2_learning_msg__srv__SetCommand_Request * msg = (ros2_learning_msg__srv__SetCommand_Request *)allocator.allocate(sizeof(ros2_learning_msg__srv__SetCommand_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ros2_learning_msg__srv__SetCommand_Request));
  bool success = ros2_learning_msg__srv__SetCommand_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ros2_learning_msg__srv__SetCommand_Request__destroy(ros2_learning_msg__srv__SetCommand_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ros2_learning_msg__srv__SetCommand_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ros2_learning_msg__srv__SetCommand_Request__Sequence__init(ros2_learning_msg__srv__SetCommand_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ros2_learning_msg__srv__SetCommand_Request * data = NULL;

  if (size) {
    data = (ros2_learning_msg__srv__SetCommand_Request *)allocator.zero_allocate(size, sizeof(ros2_learning_msg__srv__SetCommand_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ros2_learning_msg__srv__SetCommand_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ros2_learning_msg__srv__SetCommand_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ros2_learning_msg__srv__SetCommand_Request__Sequence__fini(ros2_learning_msg__srv__SetCommand_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ros2_learning_msg__srv__SetCommand_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ros2_learning_msg__srv__SetCommand_Request__Sequence *
ros2_learning_msg__srv__SetCommand_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ros2_learning_msg__srv__SetCommand_Request__Sequence * array = (ros2_learning_msg__srv__SetCommand_Request__Sequence *)allocator.allocate(sizeof(ros2_learning_msg__srv__SetCommand_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ros2_learning_msg__srv__SetCommand_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ros2_learning_msg__srv__SetCommand_Request__Sequence__destroy(ros2_learning_msg__srv__SetCommand_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ros2_learning_msg__srv__SetCommand_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ros2_learning_msg__srv__SetCommand_Request__Sequence__are_equal(const ros2_learning_msg__srv__SetCommand_Request__Sequence * lhs, const ros2_learning_msg__srv__SetCommand_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ros2_learning_msg__srv__SetCommand_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ros2_learning_msg__srv__SetCommand_Request__Sequence__copy(
  const ros2_learning_msg__srv__SetCommand_Request__Sequence * input,
  ros2_learning_msg__srv__SetCommand_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ros2_learning_msg__srv__SetCommand_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ros2_learning_msg__srv__SetCommand_Request * data =
      (ros2_learning_msg__srv__SetCommand_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ros2_learning_msg__srv__SetCommand_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ros2_learning_msg__srv__SetCommand_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ros2_learning_msg__srv__SetCommand_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `message`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
ros2_learning_msg__srv__SetCommand_Response__init(ros2_learning_msg__srv__SetCommand_Response * msg)
{
  if (!msg) {
    return false;
  }
  // success
  // message
  if (!rosidl_runtime_c__String__init(&msg->message)) {
    ros2_learning_msg__srv__SetCommand_Response__fini(msg);
    return false;
  }
  // command_id
  return true;
}

void
ros2_learning_msg__srv__SetCommand_Response__fini(ros2_learning_msg__srv__SetCommand_Response * msg)
{
  if (!msg) {
    return;
  }
  // success
  // message
  rosidl_runtime_c__String__fini(&msg->message);
  // command_id
}

bool
ros2_learning_msg__srv__SetCommand_Response__are_equal(const ros2_learning_msg__srv__SetCommand_Response * lhs, const ros2_learning_msg__srv__SetCommand_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  // message
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->message), &(rhs->message)))
  {
    return false;
  }
  // command_id
  if (lhs->command_id != rhs->command_id) {
    return false;
  }
  return true;
}

bool
ros2_learning_msg__srv__SetCommand_Response__copy(
  const ros2_learning_msg__srv__SetCommand_Response * input,
  ros2_learning_msg__srv__SetCommand_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  // message
  if (!rosidl_runtime_c__String__copy(
      &(input->message), &(output->message)))
  {
    return false;
  }
  // command_id
  output->command_id = input->command_id;
  return true;
}

ros2_learning_msg__srv__SetCommand_Response *
ros2_learning_msg__srv__SetCommand_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ros2_learning_msg__srv__SetCommand_Response * msg = (ros2_learning_msg__srv__SetCommand_Response *)allocator.allocate(sizeof(ros2_learning_msg__srv__SetCommand_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ros2_learning_msg__srv__SetCommand_Response));
  bool success = ros2_learning_msg__srv__SetCommand_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ros2_learning_msg__srv__SetCommand_Response__destroy(ros2_learning_msg__srv__SetCommand_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ros2_learning_msg__srv__SetCommand_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ros2_learning_msg__srv__SetCommand_Response__Sequence__init(ros2_learning_msg__srv__SetCommand_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ros2_learning_msg__srv__SetCommand_Response * data = NULL;

  if (size) {
    data = (ros2_learning_msg__srv__SetCommand_Response *)allocator.zero_allocate(size, sizeof(ros2_learning_msg__srv__SetCommand_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ros2_learning_msg__srv__SetCommand_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ros2_learning_msg__srv__SetCommand_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ros2_learning_msg__srv__SetCommand_Response__Sequence__fini(ros2_learning_msg__srv__SetCommand_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ros2_learning_msg__srv__SetCommand_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ros2_learning_msg__srv__SetCommand_Response__Sequence *
ros2_learning_msg__srv__SetCommand_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ros2_learning_msg__srv__SetCommand_Response__Sequence * array = (ros2_learning_msg__srv__SetCommand_Response__Sequence *)allocator.allocate(sizeof(ros2_learning_msg__srv__SetCommand_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ros2_learning_msg__srv__SetCommand_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ros2_learning_msg__srv__SetCommand_Response__Sequence__destroy(ros2_learning_msg__srv__SetCommand_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ros2_learning_msg__srv__SetCommand_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ros2_learning_msg__srv__SetCommand_Response__Sequence__are_equal(const ros2_learning_msg__srv__SetCommand_Response__Sequence * lhs, const ros2_learning_msg__srv__SetCommand_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ros2_learning_msg__srv__SetCommand_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ros2_learning_msg__srv__SetCommand_Response__Sequence__copy(
  const ros2_learning_msg__srv__SetCommand_Response__Sequence * input,
  ros2_learning_msg__srv__SetCommand_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ros2_learning_msg__srv__SetCommand_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ros2_learning_msg__srv__SetCommand_Response * data =
      (ros2_learning_msg__srv__SetCommand_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ros2_learning_msg__srv__SetCommand_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ros2_learning_msg__srv__SetCommand_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ros2_learning_msg__srv__SetCommand_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
